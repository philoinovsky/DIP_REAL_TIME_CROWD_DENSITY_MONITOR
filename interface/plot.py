...
import re
import random
from .utilities import getRoomList
...
def roomshanddler(path):
    obj = gpd.read_file(path)
    obj['x'] = obj.apply(getPolyCoords, geom='geometry',
                         coord_type='x', axis=1)
    obj['y'] = obj.apply(getPolyCoords, geom='geometry',
                         coord_type='y', axis=1)
    N = len(obj['x'])
    # setup room name according to its id

    dataList = getRoomList("s2_b4")

    # name list of the room
    obj['N'] = dataList["namelist"]

    # occupied list of the room
    obj['O'] = dataList["occupiedlist"]

    # capacity list of the room
    obj['C'] = dataList["capacitylist"]
...
def Layout(width=600, height=600):
    ...
    p.patches('x', 'y', source=handdler(buildings, "PG"),
              color='lightgrey', legend_label='buildings')
    ...
    p.multi_line('x', 'y', source=handdler(roads, "LS"),
